...
\lettrine{E}{n} este apéndice se ahonda en la planificación del proyecto, su seguimiento, el plan de gestión de riesgos empleado y se expone un análisis de los recursos y su coste.
...
\begin{itemize}
    ...
    \item Aprendizaje incremental: el sistema debe ser capaz de reconocer desconocidos (\textit{Open-Set}) e incluirlos a su base de datos (\textit{Open-World}).
...
\begin{itemize}
    \item Operación en tiempo real sujeto a la frecuencia del componente más "lento". En el caso de este proyecto, las cámaras \acrshort{rgbd} otorgan una imagen \acrshort{rgb} cada 100 ms (\textbf{10 Hz}). El objetivo es que el sistema sea capaz de mantener dicha frecuencia.
...
\begin{itemize}
    \item \textbf{Usuario o persona registrada}: individuo que acepta registrar sus datos biométricos para enriquecer el sistema.
    \item \textbf{Operador del sistema}: encargado de ejecutar y mantener el sistema.
\end{itemize}

\section{Metodología de desarrollo}
\label{sec:method}

Para el presente proyecto, se ha empleado una metodología basada en Kanban \cite{knban}. Kanban es una herramienta de gestión \textbf{visual}, cuyo objetivo es otorgar al equipo una visión global del proyecto y de su flujo de trabajo, de forma que se logra optimizar la eficiencia.

El proyecto en sí es un \textbf{tablero} en el que cada tarea representa una tarjeta, las tarjetas pasan por múltiples fases, que suponen los diferentes estados de la tarea. Normalmente, las primeras fases definen las tareas abstractas, que posteriormente se dividen en tareas más concretas a medida que se avanza en las fases. Debido a que no existe un número fijo de fases por las que tengan que pasar las tareas, se han definido las siguientes fases:
...
\begin{itemize}
    ...
    \item Implementar dichos componentes, acoplados con el procesamiento de vídeo.
...
Todos los riesgos expuestos han terminado sucediendo, excepto el del método de seguimiento, que ha logrado funcionar mejor de lo esperado en los vídeos probados. También surgieron riesgos no previstos inicialmente, como que el sistema inicial no funcionase correctamente al aplicar más de 2 cámaras.
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.75\linewidth]{imagenes/Trallao.jpg}
...
\begin{itemize}
    ...
    \item \textbf{Pruebas de los modelos y del sistema}: fue la tarea que más retrasó el proyecto. Los modelos en un principio reportaban bajas precisiones al implementarlos en \acrshort{gpu}. La mala exportación de los modelos o el pipeline de procesado de las imágenes eran las principales causas.
          ...
          Se tuvieron que rehacer todas las pruebas del sistema debido a que el nodo integrador registraba detecciones repetidas (con la misma posición \acrshort{3d}) al introducir más de 2 cámaras, lo que resultaba en más puntos sin asociar al \gls{dataset} y, por lo tanto, a una caída notoria en la precisión de detección.
...
En la tabla puede verse un desglose de los componentes utilizados y su precio \ref{tab:resources}. Todo el software utilizado en este proyecto es de uso \textbf{gratuito}, por lo que no aparece en dicha tabla. En cuanto al hardware, salvo el portátil y el router, el resto del material fue facilitado por el \acrshort{citic}, incluyendo el PC del laboratorio, cuyo coste no se ha incluido debido a no disponer de su precio de compra. Se han convertido los precios de las NVIDIA Jetson al euro que, a fecha de esta memoria, equivale a \textbf{1.2 dólares}.

\input{contenido/tables/costs.tex}

Los costes de los recursos humanos (tabla \ref{tab:humans}) requeridos para el proyecto se dividen en diferentes roles. El estudiante ha asumido los roles de jefe de proyecto, analista y programador, mientras que el rol de asesor externo lo ha asumido el tutor. Se ha estimado para el estudiante un trabajo total de \textbf{480 horas}, dividido en 350 horas para la implementación y pruebas (programador), 110 horas al análisis, diseño y documentación (analista) y 20 horas para la gestión del proyecto (jefe de proyecto). Por el lado del tutor, este ha dedicado una media de 1 hora y 30 minutos semanales al seguimiento del proyecto durante sus 46 semanas de duración (excluyendo el mes de Agosto) que, sumado a las labores de revisión de la memoria, estimadas en 20 horas, resulta en un total de \textbf{89 horas}.

Sumando los costes del material y de la mano de obra, el coste del proyecto asciende al total de \textbf{47487 €}.