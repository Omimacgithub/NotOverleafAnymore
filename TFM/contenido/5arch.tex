...
El nodo cámara recibe la información de un frame \acrshort{rgb} de una de las cámaras, que es procesado por 2 modelos de detección (expuestos en la sección \ref{sec:models}), uno destinado a rostros y el otro para cuerpos. Se obtienen \glspl{bbox} de los rostros y cuerpos detectados, que se comparan para asegurar que cada recorte facial se encuentra contenido en cada recorte corporal.

Tras comprobar que cada cara detectada se corresponde con un cuerpo, se procede al reconocimiento facial y corporal mediante los respectivos modelos (expuestos en la sección \ref{sec:models}), que devuelven un vector con las características representativas para un individuo (o \gls{embedding}).
...
En concreto, se han implementado los siguientes componentes:

\begin{itemize}
    \item \textbf{Procesamiento de secuencias de frames:} implementa toda la lógica que permite agrupar las \glspl{bbox} por individuos en más de un frame (secuencia). Todas las \glspl{bbox} se transforman en \glspl{embedding}, que se guardan en una lista para ser procesados por el módulo de \textbf{reconocimiento adaptativo}.
...
Tras agrupar las \glspl{bbox} por individuos y calcular sus vectores (\textit{\glspl{embedding}}), estos se comparan contra las personas registradas para averiguar si dichos sujetos pertenecen o no al sistema. La base de datos está compuesta por \textbf{comités (o colecciones) de \acrshort{svm} lineales} para cada individuo. Las \acrshort{svm} de los comités procesan los \textit{\glspl{embedding}}, devolviendo una serie de \textbf{puntuaciones}, que representan la distancia del punto respecto al plano lineal que distingue entre la clase del propio individuo y el resto del universo. Finalmente, se toma la decisión de reconocimiento a partir de dichas puntuaciones y se actualiza la base de datos con los nuevos \glspl{embedding}.
