...
Para cada prueba, también se mostrará la proporción de los recursos utilizados del dispositivo durante la ejecución del sistema (las pruebas se han realizado \textbf{sin el entorno gráfico}). En dispositivos Jetson, el comando \textbf{tegrastats} devuelve toda la información necesaria, en cambio, para el resto de dispositivos se obtienen de la siguiente forma:
\begin{itemize}
...
El \textit{det\_r} se muestra claramente inferior respecto a los resultados de la familia Intel, ya que el \acrshort{lidar} otorgaba cobertura completa a la reducida visión de las cámaras. Ambas Jetson logran superar el valor del equipo de referencia en esta métrica, lo que demuestra el excelente rendimiento de las \acrshort{gpu}s que integran, capaces de compensar las limitaciones de una \acrshort{cpu} de \acrshort{arm} respecto a un Intel Core i5 (procesador del equipo de referencia).

La precisión en la detección (\textit{det\_p}) empeora en la Jetson Orin Nano cuando se ejecutan 5 cámaras simultáneamente. Acorde a la tabla \ref{tab:scaleresourcesros2}, los recursos del dispositivo se encuentran al límite de procesamiento, lo que genera retardos en los resultados de los nodos. Adicionalmente, los 8 \acrshort{gb}s de memoria se vuelven insuficientes, debido a todos los recursos necesarios a reservar, que pesan en torno a cientos de \acrshort{mb}s, que se multiplican por 4 modelos y a su vez por 5 cámaras, lo que deriva en errores de ejecución por falta de memoria necesaria para realizar las inferencias. Para reducir el impacto de este problema, se han aplicado ciertas optimizaciones de los códigos de Python, que han permitido ahorrar hasta \textbf{400 \acrshort{mb}s} en la inicialización del sistema, y que se citan en el apéndice \ref{chap:optimus}.

TODO: En cambio, el \textit{det\_r} se mantiene constante. En el caso de la Jetson AGX Thor, los resultados se mantienen en la línea de la familia Intel, sin contar el bajo \textit{det\_r} causado por la ausencia del \acrshort{lidar}.

%En la Orin Nano la precision baja en casi 30 puntos mientras q el recall solo baja 1 punto... porque?? <- hay q recordar q YuNet me daba fallos de memoria

% RECALL: En la Thor con 2 y 5 cámaras el n_gt se mantiene encima de los 6000 mientras q con 10 cams baja a 4300, REFLEJAR ESTO EN LA MEMORIA.

La Jetson AGX Thor sufre una caída en el \textit{det\_p} al procesar 10 cámaras, ligeramente superior a la familia Intel. Debido al uso excesivo de \acrshort{gpu} (88\% según las pruebas), se ha trasladado la ejecución del modelo YuNet a la \acrshort{cpu}, ya que la reducción en la latencia no es tan beneficiosa al ejecutarse en CUDA (ver tabla \ref{tab:equips}). Gracias a este cambio se ha conseguido balancear la carga entre \acrshort{cpu} y \acrshort{gpu} como se muestra en la tabla \ref{tab:scaleresourcesros2} y, por consiguiente, se ha mejorado la métrica \textit{det\_p} en 5 puntos (respecto a los 57.7 de ejecutar YuNet en CUDA).

\input{contenido/tables/tests/scaleresourcesros2.tex}

En la tabla \ref{tab:scaleresourcesros2} se muestra la proporción de recursos utilizados en las pruebas del sistema en \acrshort{ros} 2. Se expone la mediana de todas las lecturas de recursos en una ejecución completa del sistema. Se emplea la mediana, ya que es robusta frente a los picos de uso de recursos.
