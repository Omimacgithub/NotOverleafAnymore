...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{imagenes/SOLAPING.jpg}
...
\begin{itemize}
    ...
    \item[Percentil (\textit{percent})] Valor del percentil aplicado en la obtención de la puntuación mínima de cada comité (). El rango probado es (25 50 75).
    ...
    \item \textbf{Número de positivos}: establece el número de frames positivos que define a un comité. El tamaño se ha fijado en 5 frames, ya que en \cite{5samples} se ha demostrado que otorga buen rendimiento, al mismo tiempo que utiliza el mínimo de muestras posibles.
...
A continuación se exponen los resultados obtenidos para cada combinación en \textit{Open-Set} y \textit{Open-World} como una media de 5 ejecuciones. A modo de obtener más puntos para definir las distribuciones de Weibull, se ha expandido el sistema con \textbf{4 individuos} a mayores de los 6 presentes en el vídeo de prueba, cuyas muestras se han adquirido de la misma forma (imágenes de las cámaras Kinect). %TODO: poner fotos??? %Se inicializa cada comité con 5 frames representativos (que corresponde con el \textbf{tamaño de la plantilla}).
