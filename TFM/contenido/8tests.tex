...
\begin{itemize}
    \item Umbral de Weibull (Tw): valor que determina si una entidad es desconocida o no. El rango probado es (0.05 0.1 0.25 0.5)
    \item Percentil: valor del percentil aplicado en la combinación de las puntuaciones generadas por las \acrshort{svm}. El rango probado es (0.25 0.5 0.75)
    \item Número de \acrshort{svm}: valor que representa al tamaño máximo de un comité. El rango probado es (1 3 7 10)
    \item Tamaño de la plantilla: establece el número de frames positivos que define a un comité. Se va a partir siempre de un tamaño de 5 frames, el mínimo necesario acorde a \cite{Erik} para definir un nuevo individuo.
    \item Tamaño de secuencia: número de frames que forman una secuencia de video. El rango probado es (5 15 25)
    \item TODO: Solapamiento: establece el salto en el número de frames entre secuencias. El rango probado es (0 5 15)
    \item Número de negativos: tamaño del conjunto de muestras negativas. El rango probado es (10 50)
    \item Umbral de actualización: valor que determina si las nuevas muestras de un individuo ya. El rango probado es (0.05 0.15 0.3)
\end{itemize}

\subsection{Resultados}
\subsection{Conclusiones}

Nneg: En la tabla (TODO) se muestra el \textit{F1 score} para 10 y 50 muestras que conforman el conjunto de negativos de entrenamiento (ratios 1:1 y 1:5 si el tamaño del template es 10). Claramente los resultados mejoran al escoger 50 negativos, esto se debe a que en 10 frames cabe un conjunto reducido de entidades a diferencia de 50 frames, sobretodo si se realiza una selección aleatoria como ocurre en este caso.
...
\section{Modo \textit{Open-Set}}
\subsection{Diseño de las pruebas}

\section{Modo \textit{Open-World}}
\subsection{Diseño de las pruebas}
