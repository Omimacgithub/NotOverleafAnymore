...
%En esta sección se explica la estructura general del sistema del proyecto. 
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.5\linewidth]{imagenes/SYSARCH.jpg}
...
Debido a que el nodo \acrshort{lidar} no está dentro del foco de este proyecto y tampoco se pudo migrar  a \acrshort{ros}2 (sección \ref{subsec:ROS2}), se ha decidido omitirlo del nuevo sistema extendido. 
...
\begin{itemize}
    ...
    \item \textbf{Reconocimiento adaptativo:} implementa la capacidad de detección e inclusión de desconocidos en el sistema (modos \textbf{\textit{Open-Set} y \textit{Open-World}}). Evalúa %, mediante el uso de teoría estadística, 
    si una \textbf{puntuación} particular para un individuo no comparte similitudes con el resto de individuos registrados. En caso afirmativo, se trata de un conocido, en caso contrario, de un desconocido. En ambos casos, el sistema actualiza su conocimiento acerca del individuo por medio de la base de datos de \textbf{personas registradas}.
...
Las \textbf{puntuaciones} utilizadas por el reconocimiento adaptativo no son más que los \textbf{resultados de los comités de \acrshort{svm}} sobre las secuencias de \glspl{embedding} de cada individuo (sección \ref{sec:val}). 
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\linewidth]{imagenes/iou.jpg}
...
\label{subsec:reiden}
Es posible que durante la detección, las \glspl{bbox} o recortes de una misma persona en frames consecutivos no se solapen debido a la velocidad de movimiento de la propia persona o a un movimiento de la cámara, lo que causa una incorrecta aplicación del método húngaro. En este caso, se asigna el mayor coste (valor de 1) a la relación (es decir, un valor de \acrshort{iou} de 0), lo que genera el riesgo de asociar 2 \glspl{bbox} de individuos diferentes. 
...
Otro problema es el no seguimiento de la persona cuando se encuentra totalmente ocluida y reaparece (ejemplo: se cruza un individuo justo delante)  o si la persona gira su cabeza y su cara queda momentánemente fuera de la visión de la cámara. 

En estos casos, se crearía una nueva entidad para el mismo individuo, lo que no es deseable. Por lo tanto, 
%Debido a etos problemas, 
es necesario aplicar un método que pueda \textbf{reidentificar} a los individuos cuyo rastro se ha perdido temporalmente. Como se comentó en la sección \ref{sec:trackerwork}, el \textbf{filtro de Kalman} no es beneficioso para la aplicación de este proyecto. Finalmente, se ha diseñado un método más sencillo basado en la \textbf{distancia euclidiana}.

Si hay un movimiento del individuo o de la cámara, o se ha realizado una asignación con el mayor coste, se calcula la distancia entre el \textbf{centro} de la \gls{bbox} actual con la \gls{bbox} posterior que no tiene solape. Si la distancia calculada es \textbf{inferior a un umbral} \textbf{se valida la asociación}. En caso contrario, la \gls{bbox} del individuo se mantiene, por si se vuelve a localizar en futuros frames. 
Si se pierde la localización del individuo, se mantiene la \gls{bbox} de la última aparición del mismo y se calcula la distancia con las \glspl{bbox} sin asociación en futuros frames. Si la distancia es \textbf{superior} a un máximo de frames (prefijado por el operador) \textbf{se abandona el rastreo}, en caso contrario se restablece.

El umbral se fija de antemano \textbf{y se ajusta automáticamente en función de la resolución} de la cámara utilizando un escalado a partir de la diagonal. 
% ==> yo esto no lo entiendo, y me parecen detalles menores de diseño ==> (se calcula la diagonal de la resolución prefijada y la nueva resolución y se divide la diagonal de la resolución nueva entre la prefijada, el resultado se multiplica por el valor de umbral). 
Siendo $\Delta x$ y $\Delta y$ la diferencia entre las coordenadas \acrshort{2d} de los centros de dos \glspl{bbox}, la distancia se calcula como %se muestra en la ecuación \ref{eq:eucl}.
\begin{equation}
...
La figura \ref{fig:ADAPTSYS} muestra los componentes del módulo de reconocimiento adaptativo: % , que se compone de los siguientes componentes:
\begin{itemize}
    ...
    \item \textbf{Módulo de actualización}: se encarga de crear un nuevo comité si se detecta un usuario nuevo (\textit{unknown}) o de crear una nueva \acrshort{svm} para registrar los cambios de un usuario ya registrado en el sistema (\textit{drift}).
...
La secuencia de vectores del individuo son procesados por el \textbf{módulo de valoración} (módulo EDF, figura \ref{fig:ADAPTSYS}), que devuelve las puntuaciones de cada comité ordenadas de menor a mayor. Así la primera puntuación (la mejor) se corresponde \textit{a priori} con el comité del individuo. A partir de las puntuaciones el \textbf{módulo de reconocimiento} (módulo RDF, figura \ref{fig:ADAPTSYS}) determina, mediante el \acrfull{evt}, si el comité corresponde realmente a un individuo (conocido) o no (desconocido). El \textbf{módulo de actualización} %(\textit{update module}, figura \ref{fig:ADAPTSYS}) 
recibe la decisión y, si el individuo es conocido, agrega una nueva \acrshort{svm} a su comité. Esa \acrshort{svm} se entrena con la secuencia de \glspl{embedding} como positivos y un extracto de muestras del resto de individuos como negativos. 
En caso de un desconocido se crea un nuevo comité con una \acrshort{svm} entrenada de la misma manera. %con el mismo conjunto de positivos y un conjunto aleatorio de las muestras de \textbf{todos los individuos} como negativos. 
Si el comité excede en el límite fijado de \acrshort{svm}s, el \textbf{módulo de limitación} %(\textit{limitation module} en la figura \ref{fig:ADAPTSYS}) 
determina la \acrshort{svm} que menos aporta a dicho comité y se \textbf{elimina}. 
%  ==> esto no es repetir lo mismo !?!?  Sendos módulos de actualización y limitación se encargan de añadir, actualizar y eliminar las \acrshort{svm}, los comités y las muestras de los individuos a la base de datos de \textbf{personas registradas}.

Para facilitar la lectura de la memoria y, puesto que no se incluyen  mejoras respecto a los modelos propuestos en \cite{Erik,CESAR}, los detalles de este módulo se han movido al apéndice \ref{chap:adaptrecon}.

\section{Personas registradas}
\label{sec:initarch}

La base de datos de todos los individuos registrados es compartida por \textbf{todo el sistema} y está formada por los comités, sus \acrshort{svm} y las muestras de cada individuo. Esta base de datos es actualizada por los módulos de actualización y limitación del reconocimiento adaptativo. A continuación se explican los diferentes procesos de \textbf{inicialización} propuestos, así como diferentes técnicas para compartir la información entre los nodos del sistema.
...
En esta primera fase del sistema se crean los registros de los individuos iniciales. Como se muestra en \cite{Erik}, el proceso de inicialización es \textbf{crítico} y depende de la calidad de las muestras escogidas. Las muestras borrosas o parcialmente ocluidas agregan confusión a las \acrshort{svm}, que devuelven puntuaciones similares al no distinguir correctamente las características del individuo.  Acorde a las pruebas de \cite{Erik} es deseable un mínimo de \textbf{5 muestras} por individuo.

Se han probado dos modos de inicializción: \textbf{semisupervisada}  (con intervención del operador) y \textbf{no supervisada} (el sistema se inicializa de manera completamente autónoma).
...
Las \glspl{bbox} se \textbf{filtran} y se descartan si no cumplen con las siguientes condiciones:
\begin{itemize}
    \item La cara se encuentra enteramente dentro del plano
    \item La \gls{bbox} es más alta que ancha.
\end{itemize}

% ==> es redundante ==> En ambos modos de inicialización, se crean los comités de cada usuario con una \acrshort{svm} inicial y los comités se añaden a la base de datos de \textbf{personas registradas}. 
%Dicha \acrshort{svm} se entrena a partir de las propias muestras del individuo (conjunto de positivos) y una selección aleatoria del resto de muestras (conjunto de negativos). Finalmente, los comités se añaden a la base de datos de \textbf{personas registradas}.
...
\textcolor{red}{queda incompleto: ventajas y desventajas, con cuál nos quedamos, etc.}