...
\subsection{Git}
Git \cite{git} es un sistema de control de versiones de \textbf{código abierto} que permite mantener un historial de las versiones de un proyecto, de forma que se mantiene la trazabilidad del mismo. GitHub es una plataforma en la nube para alojar y compartir proyectos de Git. El código y la memoria de este proyecto se ha subido a esta plataforma para su fácil acceso desde múltiples dispositivos.

\subsection{VS Code}
Visual Studio Code \cite{vscode} es un entorno de desarrollo integrado (IDE) de \textbf{código abierto}. Dicha herramienta contiene una amplia variedad de funciones integradas, como un cliente de git que facilita el control de las versiones del software y una extensión para manejar contenedores de Docker (Dev Containers), entre las funciones más usadas en este proyecto.
...
\subsection{Trello}
Trello \cite{trallao} es un programa online para la gestión de proyectos Kanban, en el que cada proyecto es un tablero con distintas listas de tarjetas que representan a las tareas. Se ha empleado para llevar un seguimiento de las tareas del proyecto en sus diferentes fases del desarrollo.

\subsection{Draw.io}
Draw.io \cite{drawio} es una herramienta online para la creación de todo tipo de diagramas, pertenece a la suite de aplicaciones de Google, lo que permite guardar y administrar los proyectos en Google Drive. Se ha utilizado para crear la mayoría de diagramas y figuras de la memoria de este proyecto.