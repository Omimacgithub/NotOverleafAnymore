...
Blackwell (2024) \cite{blackwell} es la última arquitectura diseñada para \acrshort{gpu}s de NVIDIA, en ella se incorporan los tensor cores de quinta generación, que permiten operar en las precisiones \textbf{\acrshort{fp}4} y \textbf{\acrshort{fp}6} (punto flotante de 4 y 6 bits respectivamente).
...
Los \textbf{6 dispositivos} que se emplearon en este proyecto se han agrupado en \textbf{2 familias}. Por un lado, la \textbf{familia Jetson}, que engloba a los sistemas embebidos propietarios de NVIDIA con dicho nombre y, por el otro lado, la \textbf{familia Intel}, compuesta por 1 equipo de escritorio y 1 portátil, ambos con una \acrshort{cpu} Intel junto a una \acrshort{gpu} de NVIDIA.
...
A continuación se comentan los componentes hardware sobre los que funciona el sistema del proyecto.
...
Para este proyecto, se cuenta con 2 de estas cámaras, cada una con una cobertura de visión de 57º. Dichas cámaras se disponen de forma que \textbf{no existan solapes} (en los laterales del robot, como se muestra en la figura \ref{fig:kin2}), lo que otorga un rango de visión total de \textbf{117º}.

\section{Software específico}
\label{sec:sw}

A continuación se expone el software que compone el esqueleto del sistema de este proyecto.
...
\begin{description}
    ...
    \item[PyCUDA] \cite{pycuda} Es un \gls{wrapper} para Python que interacciona con el driver de \gls{CUDA}. Se ha utilizado en conjunción con TensorRT para reservar los \glspl{stream} en los que ejecutar las inferencias, además de para reservar y realizar transferencias de memoria entre la \acrshort{cpu} y la \acrshort{gpu}.
...
A continuación, se mencionan las herramientas relacionadas con la gestión y desarrollo tanto del proyecto como de la memoria resultante (todas las herramientas, menos Trello, son de \textbf{código abierto}).
\begin{description}
