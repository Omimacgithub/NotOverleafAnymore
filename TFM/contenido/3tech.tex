...
\begin{itemize}
    ...
    \item Ampere: incorpora los tensor cores de tercera generación, que incluyen el tipo de dato \textbf{TF32}, que funciona como FP32 pero optimizado en \acrshort{gpu} sin realizar ningún cambio en el código \cite{ampere}. La \textbf{Jetson Orin Nano}, \textbf{Jetson AGX Orin} y la \textbf{GeForce RTX 3050} (gráfica del portátil) siguen esta arquitectura.
    \item Blackwell: incorpora los tensor cores de quinta generación, que permiten operar en precisión \textbf{FP4} y \textbf{FP6} (punto flotante de 4 y 6 bits respectivamente) sin apenas pérdida en la precisión, gracias a la incorporación de \textit{micro-tensor scaling}, que aplica diferentes factores de escalado a los elementos de un \gls{tensor} \cite{blackwell}. La \textbf{Jetson AGX Thor} sigue esta arquitectura.
\end{itemize}
...
\subsection{ROS}
\acrfull{ros} es un middleware de \textbf{código abierto} que incorpora las herramientas necesarias para la interacción y desarrollo de software en robots. Cuenta con una amplia biblioteca de distribuciones y librerías. Cualquier entorno de desarrollo creado a partir de este \textit{framework} está compuesto por los siguientes componentes:

\begin{itemize}
    \item
\end{itemize}

TODO: importante hablar del launch file.
