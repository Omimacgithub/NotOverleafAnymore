...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=1\linewidth]{imagenes/memcpy.png}
...
\begin{itemize}
    \item \textbf{PyCUDA} \cite{pycuda}: es un \gls{wrapper} que interacciona con el driver de CUDA. \textbf{CUDA (\textit{Compute Unified Device Architecture})} es un conjunto de herramientas, que incluye un compilador para compilar código en lenguaje CUDA, que es una variante del lenguaje C. Dicho código permite la ejecución de algoritmos en \acrshort{gpu}s de propósito general de NVIDIA. Se ha utilizado principalmente para generar los contextos y reservar la memoria en la \acrshort{gpu}, con el propósito de realizar las inferencias de los modelos.
...
\begin{itemize}
    ...
    \item \textbf{OpenCV} \cite{OpenCVCUDA}: el módulo de redes neuronales profundas (dnn) incluye modelos ya entrenados y un \gls{rt} para realizar inferencias. Dicho módulo puede ejecutarse en \textbf{CUDA}, solo si se compila el código fuente. En este proyecto se ha utilizado un modelo de detección de caras ofrecido por dicha librería.
\end{itemize}
