...
A modo de evaluar el rendimiento en términos de precisión del sistema y de los modelos de visión artificial vistos en los capítulos \ref{chap:cnn} y \ref{chap:systesting}, se parte de un \gls{dataset} creado en \cite{andrew}. El \gls{dataset} contiene datos etiquetados extraídos de un fichero Rosbag, que contiene la información de las dos cámaras Kinect \acrshort{rgbd} y del sensor \acrshort{lidar} instalados en el Summit\_XL durante la realización de una prueba. Dicha prueba se ha realizado a partir de los datos grabados de una prueba de 2 minutos dentro del laboratorio de robótica del \acrshort{citic} con 6 personas. Cada entrada en el fichero \acrshort{json} del \gls{dataset} se compone de los siguientes objetos \cite{andrew}:
