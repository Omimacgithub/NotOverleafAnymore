...
\textbf{Tareas:}
\begin{itemize}
    \item Instalación de TensorRT, PyCUDA, entre otras librerías en los dispositivos.
...
Todos los riesgos expuestos han terminado sucediendo, junto a otros riesgos no previstos que surgieron durante el proyecto (ejemplo: los archivos de las pruebas no pueden almacenarse de forma local en el dispositivo por problemas de espacio).
...
\begin{itemize}
    ...
    \item \textbf{Pruebas de los modelos y del sistema}: fue la tarea que más retrasó el proyecto. Los modelos en un principio reportaban bajas precisiones al implementarlos en \acrshort{gpu}. La mala exportación de los modelos o el pipeline de procesado de las imágenes eran las principales causas.
          ...
          Se intentó convertir el modelo YuNet para utilizarse en el \textit{backend} de TensorRT pero, debido al complejo procedimiento del procesado de las entradas y salidas, se mantuvo el modelo en su versión de OpenCV, ya que de la otra forma no se lograron resultados equiparables.
    ...
    \item \textbf{Diseño e implementación del sistema adaptativo}: en un principio fue complicado comprender todas las bases de la arquitectura de referencia \cite{Erik}. Tras empezar la dedicación a tiempo completo al proyecto en Octubre, se pudo cumplir con el objetivo el mes siguiente.
    \item \textbf{Pruebas del sistema extendido}: esta tarea fue según el tiempo previsto, ya que el rendimiento del sistema evolucionaba rápidamente conforme se iban añadiendo mejoras. Aunque fue necesario contar con más personas, con un total de 10 fueron suficientes (4 más que en el \gls{dataset} de las pruebas) para realizar unas pruebas razonables del sistema.
    \item \textbf{Análisis de nuevos dispositivos}: esta tarea también llevo más tiempo de lo previsto, ya que tuvieron que rehacerse las pruebas de modelos en \textbf{todos los dipositivos}, debido a modificaciones en los modelos que reducían la latencia y el cambio de versión del modelo YOLO8n a YOLO11n.
\end{itemize}
